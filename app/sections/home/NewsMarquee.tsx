...
export const NewsMarquee = () => {
  // 1. Combine Data
  const announcements: CombinedItem[] = sortedAnnouncements.slice(0, 4).map(a => ({
    id: a.id,
    type: 'Announcement',
    title: a.title,
    date: a.date,
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="absolute top-0 left-0 right-0 z-20 p-6 pb-4 border-b border-[var(--border-highlight)]" style={{ background: 'rgba(20, 20, 20, 0.4)', backdropFilter: 'blur(10px)' }}>
      ...
      <div className="relative flex-1 overflow-hidden mt-20 group">
        ...
        >
          {loopArray.map((item, index) => (
            <Link key={`${item.id}-${index}`} href={item.link} className="block group/card">
              <div 
              ...
              >
                <div className="flex justify-between items-start mb-2 gap-4">
                  <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full border ${item.type === 'Announcement' ? 'border-[var(--color-highlight)] text-[var(--color-highlight)]' : 'border-[var(--accent-3)] text-[var(--accent-3)]'}`}>
                    {item.type}
                  </span>
                  {item.isImportant && (
